...
<body id="pCont">
  ...
  if ($method == 'POST') {
    // echo "post meth";
    //this will elaborate connections aggregation connectipn pipelines
    ...
    function calculatePrice($basePrice, $pCount, $disCountRates)
    {
      if ($pCount <= 0) {
        ...
        $discount = $disCountRates[$pCount];//added in array count 
      }
      $discountedPrice = $basePrice * (1 - $discount);
      return $discountedPrice;
    }
    $payableAmt = calculatePrice($basePrice, $pCount, $disCountRates);//this is payable amount 
    $email;
    ...
    //this will calcuoate the discount price as per the no of persons selected as per the coint generated by an approopriate persopn
   //data is isolated as an specialuser and each data is seperate for each user taht can be manageed by an appropropriate meanings


   //main aspects
   //1)security group
   //2)person count as per regstreaiijt;s
   //3)meaning lessr sjhls;lhyw;nq

    echo $email;
  $sql = "INSERT INTO `userdetails`( `user`, `package`,`schedule`, `status`, `email`,`persons`,`payable amount`) VALUES ('$sessionUser','$package','$schedule','$status','$email','$pCount','$payableAmt')";//this ensuere payment details email will be sent to ab client 
    if (isset($_SESSION['userName'])) {
      $res = mysqli_query($con, $sql);//this will ensure sql query tarnsforms as per the law of enforcement policies as queries will not be managed by external user 
    } else {
    ...
    //security groups payload 
    //in this case the person will able to manage thww ooohs 
  }
